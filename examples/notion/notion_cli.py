...
@cli.command()
@click.option('--config', '-c', type=click.Path(exists=True), help='Caminho para arquivo de configuração')
@click.option('--token', envvar='NOTION_TOKEN', help='Token de integração do Notion')
@click.option('--workspace', envvar='NOTION_WORKSPACE', help='ID do workspace do Notion')
@click.option('--source', '-s', type=click.Path(exists=True), help='Diretório fonte para sincronização')
...
@cli.command()
@click.option('--token', envvar='NOTION_TOKEN', required=True, help='Token de integração do Notion')
@click.option('--output', '-o', type=click.Path(), default='notion_config.json', help='Arquivo de saída')
...
@cli.command()
@click.option('--config', '-c', type=click.Path(exists=True), required=True, help='Arquivo de configuração')
def validate(config):
    ...
    try:
        ...
        for mapping in config_data['mappings']:
            if 'source_path' not in mapping or 'target_id' not in mapping:
                click.echo('✗ Mapeamento inválido: necessário source_path e target_id', err=True)
                return
