...
from .types import (
    ...
    NotionBlock, NotionPage, NotionDatabase,
    NotionObjectType, BlockType, PropertyType,
    MARKDOWN_BLOCK_MARKERS, API_VERSION,
    DEFAULT_CACHE_TTL, DEFAULT_BATCH_SIZE
)
...
class NotionClient:
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse) -> Dict[str, Any]:
        ...
        self._rate_limits['remaining'] = int(response.headers.get('x-ratelimit-remaining', 100))
    ...
    async def _convert_to_notion_page(self, data: Dict[str, Any]) -> NotionPage:
        ...
        return NotionPage(
            ...
            title=data["properties"]["title"]["title"][0]["plain_text"] if data.get("properties", {}).get("title") else "",
            parent=data["parent"],
    ...
    async def update_page(self, page_id: str, content: str = None, properties: Dict = None) -> NotionPage:
