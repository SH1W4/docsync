...
from docsync.integrations.notion import (
    NotionClient, NotionBridge, NotionConfig, NotionMapping,
    NotionPage, NotionDatabase, NotionBlock,
    NotionError, NotionAuthError, NotionRateLimitError
)
...
def test_data():
    ...
    return {
        "pages": {
            ...
            "with_content": {
                "id": "content_page_id",
                "properties": {"title": {"title": [{"text": {"content": "Content Page"}}]}},
                "parent": {"page_id": "parent_id"},
                "children": [
                    {"type": "paragraph", "paragraph": {"text": [{"text": {"content": "Test content"}}]}},
                    {"type": "code", "code": {"text": [{"text": {"content": "print('test')"}}], "language": "python"}}
                ]
            }
        },
        "responses": {
...
class TestNotionClient:
    ...
    async def test_verify_connection(self, notion_client, mock_response):
        ...
        with patch("aiohttp.ClientSession.request", AsyncMock(return_value=mock_response)):
    ...
    async def test_cache_behavior(self, notion_client, mock_api_response, test_data):
        ...
        with patch("aiohttp.ClientSession.request", AsyncMock(side_effect=responses)) as mock_request:
            # Primeira chamada
            ...
            result4 = await notion_client._request("GET", "test", payload={"diff": True}, use_cache=True)
...
class TestNotionBridge:
    ...
    async def test_sync_file_to_notion(self, notion_bridge, tmp_path):
        ...
        with patch.object(notion_bridge.client, "create_page", AsyncMock()) as mock_create:
    ...
    async def test_sync_page_to_local(self, notion_bridge, tmp_path):
        ...
        with patch.object(notion_bridge.client, "get_page", AsyncMock(return_value=mock_page)):
            with patch.object(notion_bridge.client, "get_block_children", AsyncMock(return_value={"results": []})):
                await notion_bridge.sync_page_to_local("test_id")
                assert (tmp_path / "test.md").exists()
                
    @pytest.mark.asyncio
    async def test_conflict_resolution_detailed(self, notion_bridge, file_helper, test_data):
        ...
        with patch.object(notion_bridge.client, "get_page", AsyncMock(return_value=notion_version)) as mock_get:
            with patch.object(notion_bridge.client, "update_page", AsyncMock()) as mock_update:
                with patch.object(notion_bridge, "_get_local_version", return_value=local_version):
                    ...
                    mock_update.reset_mock()
                    with patch.object(notion_bridge, "_merge_changes", AsyncMock()) as mock_merge:
                        await notion_bridge.sync_file_to_notion(str(local_file), force_merge=True)
...
class TestIntegration:
    ...
    async def test_full_sync_flow(self, notion_bridge, tmp_path):
        ...
        with patch.object(notion_bridge.client, "create_page", AsyncMock(return_value=mock_page)):
            with patch.object(notion_bridge.client, "get_page", AsyncMock(return_value=mock_page)):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_and_recovery(self, notion_bridge, file_helper, test_data):
        ...
        with patch.object(notion_bridge.client, "_request") as mock_request:
            mock_request.side_effect = [
                ...
                AsyncMock(return_value=test_data["responses"]["success"])()  # Segunda tentativa sucede
            ]
            await notion_bridge.sync_file_to_notion(str(test_file))
            assert mock_request.call_count == 2
        
        # Teste 2: Erro de autenticação
        with patch.object(notion_bridge.client, "_request", side_effect=NotionAuthError):
    ...
    async def test_validation_and_limits(self, notion_bridge, file_helper):
        ...
        invalid_content = file_helper.create_markdown("invalid.md", "```\nUnclosed code block")
    ...
    async def test_permission_handling(self, notion_bridge, file_helper, test_data):
        ...
        with patch.object(notion_bridge.client, "get_page", side_effect=NotionAuthError("no read access")):
            with pytest.raises(NotionAuthError, match="no read access"):
                await notion_bridge.sync_page_to_local("test_id")

        # Teste 2: Sem permissão de escrita
        with patch.object(notion_bridge.client, "create_page", side_effect=NotionAuthError("no write access")):
    ...
    async def test_concurrent_sync(self, notion_bridge, tmp_path):
        ...
        with patch.object(notion_bridge.client, "create_page", AsyncMock()):
            tasks = [
                notion_bridge.sync_file_to_notion(str(f))
                for f in files
            ]
