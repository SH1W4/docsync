...
class NotionSyncMonitor:
    ...
    async def _poll_notion_changes(self):
        ...
        while self._running:
            try:
                for mapping in self.config.mappings:
                    changes = await self.bridge.client.get_recent_changes(mapping.target_id)
                    if changes:
                        self.stats.changes_notion += len(changes)
                        self.stats.notion_queue.update(
                            page['id'] for page in changes
                        )
    ...
    def _create_status_table(self) -> Table:
        ...
        table.add_row('Dados Transferidos', f"{self.stats.bytes_transferred/1024:.2f} KB")
