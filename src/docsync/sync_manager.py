...
class VersionController:
    ...
    def __init__(self, config: Dict):
        ...
        self.commit_template = config.get('commit_message_template', 
                                        "doc: {action} - {path}")
...
class FileSystemMonitor(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if event.is_directory:
            return
        asyncio.create_task(
            self.sync_manager.handle_file_change(Path(event.src_path))
        )
...
class SyncManager:
    ...
    def __init__(self, config: Config):
        ...
        self.version_control = VersionController(
            self.guardrive_config.version_control
        )
    ...
    def _should_ignore(self, file_path: Path) -> bool:
        ...
        return any(
            pattern in name
            for pattern in self.sync_config.ignore_patterns
        )
    ...
    def _find_mapping_for_file(self, file_path: Path) -> Optional[PathMappingConfig]:
        ...
        for mapping in self.guardrive_config.path_mappings:
            if (str_path.startswith(mapping.source_path) or 
                str_path.startswith(mapping.target_path)):
                return mapping
        return None
        
    def _get_target_path(self, source_path: Path, 
                        mapping: PathMappingConfig) -> Path:
    ...
    async def _sync_directory_pair(self, source: Path, target: Path, 
                                 doc_type: DocumentType):
