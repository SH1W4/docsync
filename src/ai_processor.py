...
class DocumentProcessor:
    ...
    def _evaluate_structure(self, content: str) -> float:
        ...
        is_hierarchical = all(a <= b for a, b in zip(section_levels, section_levels[1:]))
    ...
    def _check_code_quality(self, content: str) -> float:
        ...
        for block in code_blocks:
            ...
            lines = block.split('\n')
            if len(lines) > 1:
                indentation_consistent = len(set(len(line) - len(line.lstrip()) 
                                             for line in lines if line.strip())) <= 2
