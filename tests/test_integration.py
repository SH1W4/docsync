...
from src.ai_processor import DocumentProcessor, AIEnhancedMonitor
...
def test_markdown_processing(temp_doc_dir: Path):
    ...
    assert quality['metrics']['completeness'] == 1.0  # Todas seções requeridas presentes
...
def test_file_monitoring(temp_doc_dir: Path):
    ...
    try:
        ...
        # Modificar arquivo
        test_file.write_text(pytest.test_doc_content + "\n## New Section", encoding='utf-8')
...
def test_batch_processing(temp_doc_dir: Path):
    ...
    (temp_doc_dir / "config1.yaml").write_text(pytest.test_yaml_content, encoding='utf-8')
