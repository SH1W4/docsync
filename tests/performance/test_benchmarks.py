...
class TestSyncPerformance:
    ...
    def test_file_processing_speed(self, benchmark, temp_dir: Path, sample_markdown: str):
    ...
    async def test_concurrent_sync_performance(
        self, temp_dir: Path, sample_config: dict, performance_config: dict
    ):
    ...
    def test_large_file_processing(
        self, benchmark, temp_dir: Path, performance_config: dict
    ):
...
class TestNotionIntegrationPerformance:
    ...
    @pytest.mark.asyncio
    async def test_api_request_batching(self, mock_notion_client, performance_config: dict):
        ...
        pages_data = [
            {"title": f"Page {i}", "content": f"Content {i}"}
            for i in range(100)
        ]
